...
Paul, after reading the paper and your comments (since my copy of hash-book is still in shipping and I can't read examples 10.4 and 11.16) I decided to play with the hash tables and write myself a code that extracts duplicates in one datastep (lets call it
...
5) as Paul wrote in the paper, that RAM might be a bottle neck for hash solution, I did some additional test (in which used T_ID variable from input dataset instead of Test_ID) so there was only one big BY group (~80M obs.), in that case hash worked ~230s.
...
This trick was born out of sheer practical necessity while doing work for a client. The task was to clean up an ETL process pre-aggregating stuff for online reporting. The corporate DW from which the data were extracted was, uh, dusty; and so there were se
his thread).

Each contained a small number (relative to the total obs) of dupes by the DW insertion date and surrogate key. They needed to be killed before the processes could go forward, so the sort -> dedup - resort had been being applied to every such data set, whic
be run overnight to make the data available for analysis every day in the morning, and it simply would not fit in the production window. The other factor was constantly running out of utility sort work disk space and having to deal with the system people t

After examining the entire ETL (similar for each extract), it was clear that the narrowest bottleneck was in killing the dupes, so I concocted this method and tested it. Turned out, it was able to reduce the dupe killing time to about 20 minutes for all th
uch smaller than 1 TB, particularly if there're many to deal with. Needless to say, I encapsulated the thing in a macro parameterized to account for a number of scenarios - for example, the client wanted the option of saving the dupes separately for furthe
can easily ideate its essence).

On a different note, the same can be done in a single DATA step without explicit sorting at all by using a hash table to identify the dupes to kill using a variation of the program 10.4 (or 11.16) shown in our hash book. I didn't opt for it at the time bec
